...
class DiffTf(Node):
    ...
    def __init__(self):
        ...
        self.joint_state_pub = self.create_publisher(JointState, 'joint_states', QoSProfile(depth=10))
    ...
    def update(self):
        ...
        joint_state_msg.name = ['drivewhl_l_joint', 'drivewhl_r_joint']
        joint_state_msg.position = [self.left, self.right] # 실제로 1 encoder 값이 증가할 때 얼마나 각도가 돌아가는 만큼 수정해야함.
        self.joint_state_pub.publish(joint_state_msg)
        self.get_logger().info("Right Wheel: {0}, Left Wheel: {1}".format(self.right, self.left))
